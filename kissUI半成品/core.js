...
window.KSN = (function () {
  ...
  var PLACEHOLDERS = {
    FOX_SPLASH_CHARACTER:            { label: '开屏页·蓝狐狸角色素材', where: '入口页 · logo 动画后的左侧', size: '112×112' },
    SHEEP_SPLASH_CHARACTER:          { label: '开屏页·绿羊角色素材',   where: '入口页 · logo 动画后的右侧', size: '112×112' },
    ...
    FOX_HOME_CHARACTER:              { label: '人人星卡片图素材',      where: '首页 · 人人星入口卡',      size: '卡片图' },
    ...
    SHEEP_CHARACTER_RESERVED:        { label: '小羊羔·小机星卡片图素材', where: '首页 · 小机星入口卡（暂不入核心聊天页）', size: '卡片图' },
  ...
  /* ---------- 4. 素材占位块 ---------- */
  /* ph(code, {h, w, size, round, compact}) —— 始终带 data-placeholder 便于检索 */
  ...
  function field(o) {
    ...
    return '<div class="' + cls.join(' ') + '">' + label
      ...
      +   '<input class="field__input" type="text" value="' + esc(o.value || '') + '" placeholder="' + esc(o.placeholder || '') + '">'
      + '</div>' + msg + '</div>';
  }
