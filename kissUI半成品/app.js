...
(function () {
  ...
  function doConnect() {
    /* 从连接页表单读取输入 */
    var codeInput = document.querySelector('.field__input[placeholder="6 位配对码"]');
    var serverInput = document.querySelector('.field__input[placeholder="host:port"]');

    var code = codeInput ? codeInput.value.trim() : '';
    var server = serverInput ? serverInput.value.trim() : '';

    if (!code) {
      nav('#/connect?state=empty');
      return;
    }
    if (!server) {
      nav('#/connect?state=empty');
    ...
    /* 先切到"正在连接"状态 */
    nav('#/connect?state=connecting');

    /* 调用后端 API */
    K.backend.pair(code, server, function (err, data) {
      if (err) {
        var msg = err.message || '';
        if (msg.indexOf('401') >= 0 || msg.indexOf('配对码') >= 0) {
          nav('#/connect?state=code-error');
        } else if (msg.indexOf('网络') >= 0 || msg.indexOf('fetch') >= 0) {
          nav('#/connect?state=network-error');
        } else {
          nav('#/connect?state=code-error');
        }
        return;
      ...
      /* 配对成功，获取对话历史 */
      K.backend.bootstrap(function (err2, bootstrapData) {
        if (!err2 && bootstrapData && bootstrapData.history) {
          /* 把历史消息加载到 CHAT_LOG */
          /* 注意：这里需要访问 screens-a.js 的 CHAT_LOG，
             但为了最小改动，先跳过，后续可以优化 */
        }
        nav('#/connect/success');
      });
