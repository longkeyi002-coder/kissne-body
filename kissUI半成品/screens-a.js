...
(function () {
  ...
  /* =====================================================================
     ...
     ===================================================================== */
  var HOME_STATES = [
    { key: 'online',  label: '设备在线' },
    { key: 'offline', label: '设备离线' }
  ];

  K.registerScreen({
    no: '04', id: 'home', name: '首页 / 控制台', route: '#/home', tab: 'entry',
    purpose: '手机桌面式入口页。最上方左侧「人人星」→ 聊天页，右侧「小机星」→ AI World；其余功能以手机桌面式图标网格排列。只有桌面内容，不做列表式的最近活动。白蓝配色。',
    ...
    render: function (ctx) {
      ...
      /* —— 最上方：人人星 ｜ 小机星，左右各占一半，图片素材类型 —— */
      /* 两张星卡**只有素材**：不放名称、不放箭头。
         哪一个是谁，靠素材本身辨认；点素材就进对应页面。 */
      var stars = `
        <div class="starcards">
          <div class="starcard${offline ? ' is-disabled' : ''}"${offline ? '' : ' data-nav="#/chat"'}>
            <div class="starcard__art">
              ${ph('FOX_HOME_CHARACTER')}
              ${icon('paw', 32, 'starcard__motif')}
            </div>
          </div>
          <div class="starcard starcard--sheep" data-nav="#/universe">
            <div class="starcard__art">
              ${ph('SHEEP_CHARACTER_RESERVED')}
              ${icon('hoof', 32, 'starcard__motif')}
            </div>
          </div>
        </div>`;
  ...
  /* 消息两侧都带头像：AI 一侧是叶青栩的头像占位，我这一侧是用户头像占位。
     每条消息都带时间戳 —— 真实聊天不可能没有时间。
     **tag** 是头像占位上的文字：叶青栩做事时它会跟着变（思考中 / 联网中 / 看日程…），
     真机上换成对应的表情 / 动图素材即可。 */
  function ava(code, tag) {
    return '<div class="msg__ava">' + ph(code, { size: 34, compact: true, tag: tag || '头像', round: true }) + '</div>';
  }
  function aiMsg(html, cls, time, tag) {
    return '<div class="msg msg--ai">' + ava('FOX_CHAT_AVATAR', tag)
      + '<div class="msg__body"><div class="msg__text' + (cls ? ' ' + cls : '') + '">' + html + '</div>'
      + '<span class="msg__time">' + (time || '09:41') + '</span></div>'
      + '</div>';
  }
  function meMsg(html, meta, time) {
    return '<div class="msg msg--me">' + ava('USER_AVATAR')
  ...
  /* 工具调用卡片（示例）：工具名 + 参数 / 返回 / 状态 */
  function toolCard(name, rows, cost) {
    return '<div class="toolcall">'
      + '<div class="toolcall__head">调用工具 · ' + name + '（示例）</div>'
      + rows.map(function (r) {
          return '<div class="toolcall__row"><span>' + r[0] + '</span><b>' + r[1] + '</b></div>';
        }).join('')
      + '<div class="toolcall__row"><span>状态</span><b>已完成 · ' + (cost || '0.8s') + '</b></div>'
      + '</div>';
  }
  ...
  K.registerScreen({
    ...
    render: function (ctx) {
      ...
      var typing = s === 'keyboard';  /* 键盘弹出态：输入区整组抬起，露出透明的键盘区 */
      ...
      if (bs !== 'empty') {
        /* 头像占位上的文字 = 叶青栩此刻在做什么（真机上换成对应表情 / 动图素材） */
        if (bs === 'sending')  base += meMsg('今天天气不错', '<span class="msg__meta-in">发送中…</span>', '09:42');
        if (bs === 'typing')   base += aiMsg('正在输入' + dots(), 'is-pending', '09:42', '打字');
        if (bs === 'thinking') base += aiMsg('正在思考' + dots(), 'is-pending', '09:42', '思考');
        if (bs === 'replying') base += aiMsg('嗯，我在听——<span class="caret"></span>', '', '09:42', '说话');
        if (bs === 'failed') {
          base += aiMsg(icon('alert', 15) + '<span>回复失败，设备未响应。</span>'
            + btn('重新发送', { small: true, kind: 'ghost', action: 'resend' }), 'is-failed', '09:42');
        }
        /* 思考链（示例）：AI 回复上方挂一块灰底的思考过程，
           内容是占位示例文本，真实思考内容由 Hermes 输出 */
        if (bs === 'cot') {
          base += aiMsg('<div class="cot is-open">'
            + '<div class="cot__head">已深度思考（示例）· 用时 3.2s</div>'
            + '<div class="cot__body">'
            + '<div class="cot__line">用户在问我有没有在</div>'
            + '<div class="cot__line">先简短回应，再主动问一句</div>'
            + '<div class="cot__line">语气保持轻松，不啰嗦</div>'
            + '</div></div>嗯，我在听。今天想做点什么？', '', '09:42', '深思');
        }
        /* —— 工具调用（示例）四个例子 ——
           AI 做事时**头像上的文字会跟着变**：查记忆 / 联网 / 看日程 / 写便签。
           参数与返回值都是占位，真实调用由 Hermes 工具链完成。 */
        if (bs === 'tool') {
          base += aiMsg(toolCard('记忆检索', [
              ['参数', '关键词 = 「周末计划」'],
              ['返回', '待接入 · 由 Hermes 工具返回']
            ], '0.8s'), '', '09:42', '记忆')
            + aiMsg('找到了，你之前提过周末想整理房间——这就要开始吗？', '', '09:42', '说话');
        }
        if (bs === 'tool-web') {
          base += aiMsg(toolCard('联网搜索', [
              ['参数', '关键词 = 「周末 天气」'],
              ['返回', '待接入 · 由 Hermes 工具返回']
            ], '1.2s'), '', '09:42', '联网')
            + aiMsg('我查了：周六晴、周日下午有雨——「看展」放周六更合适。', '', '09:43', '说话');
        }
        if (bs === 'tool-cal') {
          base += aiMsg(toolCard('查看日程', [
              ['参数', '范围 = 本周六 ～ 周日'],
              ['返回', '待接入 · 由 Hermes 工具返回']
            ], '0.4s'), '', '09:42', '日程')
            + aiMsg('你周六上午空着。要我先把「美术馆」占上吗？', '', '09:43', '说话');
        }
        if (bs === 'tool-write') {
          base += aiMsg(toolCard('写备忘录', [
              ['内容', '「周六去看展」'],
              ['结果', '待写入 · 需你确认（待接入）']
            ], '0.6s'), '', '09:42', '便签')
            + aiMsg('写好了，放在备忘录第一条。要改随时说。', '', '09:43', '说话');
        }
        /* 小机星进入请示：人类不能自己闯进小机星，得先在人人星跟叶青栩打招呼。
           批准后才会出现绿色小人 + 方向键（见 06 小机星页的 state=enter）。 */
        if (bs === 'request-enter') {
          base += aiMsg('你想来小机星看看？可以，我开门给你。', '', '09:42', '开门');
        }
      }
      ...
      var stkPanel = panel === 'sticker' ? '<div class="stkpanel">'
        ...
        + STICKERS.map(function (s) {
            return '<button class="stkpanel__i" type="button" data-stk="' + s.k + '"'
              + ' data-placeholder="STICKER_PACK">' + esc(s.t) + '</button>';
          }).join('')
        + '</div>'
        + '<div class="stkpanel__foot"><span>两排 · 左右滑动看更多</span>'
      ...
      <div class="screen screen--chat${typing ? ' is-typing' : ''}">
        ...
        <!-- 输入区整组（表情面板 + 磁吸快捷条 + 输入框）：打字时一起抬起；
             框外的「键盘区域」保持透明，只留一条虚线，不铺底色。 -->
        ...
        <div class="keyboard" data-kbd><span>键盘区域（占位）</span></div>
        ${menuLayer}
    ...
    mount: function (root, ctx) {
      ...
      function push() {
        ...
        /* 检查是否已连接后端 */
        if (K.backend && K.backend.isConnected()) {
          /* 真实 API 调用 */
          var aiEl = aiMsg('', 'is-pending', clockNow(), '思考');
          append(aiEl);
          var bodyEl = aiEl.querySelector('.msg__body');

          K.backend.sendMessage(v, function (err, data) {
            if (err) {
              if (bodyEl) bodyEl.innerHTML = '<span class="msg__error">发送失败：' + esc(err.message) + '</span>';
              pushLog({ who: 'ai', html: '发送失败：' + esc(err.message), time: clockNow() });
              paintPill();
              return;
            }
            /* 发送成功，开始轮询 */
            if (!pollingActive) {
              pollingActive = true;
              K.backend.startPolling(function (evt) {
                handleEvent(evt);
              });
            }
          });
          ...
          replyT = setTimeout(function () {
            var t = clockNow();
            var html = '（占位回复）我收到了：' + esc(v);
            append(aiMsg(html, 'is-pending', t, '回复'));
            pushLog({ who: 'ai', html: html, time: t });
            paintPill();
          }, 700);
        }
      }
      ...
      function handleEvent(evt) {
        if (evt.type === 'pending') {
          /* AI 开始思考 */
          if (currentBodyEl) currentBodyEl.innerHTML = '<span class="msg__thinking">思考中…</span>';
        } else if (evt.type === 'delta') {
          /* 增量文本 */
          if (!currentBodyEl) {
            currentAiEl = aiMsg('', 'is-pending', clockNow(), '说话');
            append(currentAiEl);
            currentBodyEl = currentAiEl.querySelector('.msg__body');
            currentText = '';
          }
          currentText += evt.content || '';
          if (currentBodyEl) currentBodyEl.innerHTML = esc(currentText);
        } else if (evt.type === 'completed') {
          /* 完成 */
          if (currentBodyEl && currentText) {
            currentBodyEl.innerHTML = esc(currentText);
          }
          pushLog({ who: 'ai', html: currentText || '（空回复）', time: clockNow() });
          currentAiEl = null;
          currentBodyEl = null;
          currentText = '';
          paintPill();
          ...
          /* 取消 */
          if (currentBodyEl) {
            currentBodyEl.innerHTML = '<span class="msg__cancelled">已取消</span>';
          }
          currentAiEl = null;
          currentBodyEl = null;
          currentText = '';
      ...
      function onStkTap(e) {
        var s2 = pick(STICKERS, e.currentTarget.getAttribute('data-stk'), 's1');
        var html = '<span class="stkmsg" data-placeholder="STICKER_PACK">' + esc(s2.t) + '</span>';
      ...
      if (skKey && LAST_SENT_HASH !== location.hash) {
        ...
        var sk = pick(STICKERS, skKey, 's1');
        var html = '<span class="stkmsg" data-placeholder="STICKER_PACK">' + esc(sk.t) + '</span>';
        append(meMsg(html, '', clockNow()));
        pushLog({ who: 'me', html: html, time: clockNow() });
      }
  ...
  function sceneView(roomKey, touchKey) {
    ...
    return '\n      <div class="screen screen--scene' + (sc.out ? ' is-out' : '') + '">'
      ...
      + (aiHere ? '\n            <div class="scn__actor">' + sprite('') + '</div>' : '')
      + '\n          </div>'
      + '\n          <div class="scn__say">'
        ...
        <div class="call">
          ${share
            ? '<div class="call__share">'
              ...
              + '</div>'
            : '<div class="call__ava">' + ph('FOX_CHAT_AVATAR', { size: 118, tag: '叶青栩素材' }) + '</div>'}
  ...
  K.registerScreen({
    ...
    render: function (ctx) {
      ...
      /* 从**首页**进来是"管理"：点一张只是选中，**不跳转**到人人星；
         从**聊天「＋」**进来才是"挑一张发出去"。 */
      var tiles = STICKERS.map(function (s) {
        var inner = '<span class="stk__art" data-placeholder="STICKER_PACK">' + esc(s.t) + '</span>'
          + '<span class="stk__size">240×240</span>';
        return fromChat
          ? '<a class="stk" data-nav="#/chat?state=replying&sticker=' + s.k + '&n=' + nonce + '">' + inner + '</a>'
          : '<button class="stk" type="button" data-pick="' + s.k + '">' + inner + '</button>';
      }).join('');
      ...
      <div class="screen">
        ...
        <div class="screen__body">
          ${fromChat ? '' : card(
            ...
            + '</div>'
          , { tight: true })}
          ${sectionTitle('我的表情包', '<span class="muted">' + STICKERS.length + ' 张 · 占位</span>')}
          <div class="stkgrid">${tiles}</div>
          ${fromChat
          ...
          ${fromChat
            ...
            : note('表情包是你自己上传的素材（占位代码 STICKER_PACK，规格见上）；发送入口在人人星输入栏上方的「表情包」。')}
        </div>
      </div>`;
    },
    ...
    mount: function (root) {
      ...
      function onPick(e) {
        ...
        if (hint) {
          var name = el.querySelector('.stk__art');
          hint.textContent = '已选中「' + ((name && name.textContent) || '').trim()
            + '」；要发出去请用人人星输入栏上方的「表情包」。';
        }
      }
