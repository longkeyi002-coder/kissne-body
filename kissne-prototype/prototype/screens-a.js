...
(function () {
  ...
  function assistantBubbleHtml(html, cls) {
    ...
    var parts = [], current = [];
    function flush() { var part = current.join('\\n').trim(); if (part) parts.push(part); current = []; }
    ...
    return parts.map(function (part) { return '<div class="msg__text bubble' + (cls ? ' ' + cls : '') + '">' + part + '</div>'; }).join('');
  }
