...
class KissneMobileAdapter(BasePlatformAdapter):
    ...
    def _payload_fingerprint(text: str, attachments: Optional[List[Dict[str, Any]]] = None) -> str:
        """Digest of user-visible inbound content, including attachment identity.

        Labels are metadata; the media bytes themselves are hashed so changing an image while reusing a
        client message_id is a conflict, just like changing text.
        """
        items = []
        ...
        canonical = json.dumps(
            {"text": text, "attachments": items}, ensure_ascii=False,
    ...
    def _decode_attachments(body: Dict[str, Any]) -> Tuple[Optional[List[Dict[str, Any]]], Optional[str]]:
        """Validate the bounded image/sticker wire format and decode it in memory."""
    ...
    def _materialize_attachments(attachments: List[Dict[str, Any]]) -> List[str]:
        """Write validated media to private temporary files for Hermes' existing vision path."""
        suffixes = {
    ...
    async def _handle_inbound(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            event = MessageEvent(
                ...
                source=source,
                raw_message=body,
                # Hermes' established multimodal path consumes local media files.  The generated
                # vision description stays internal; Mobile never substitutes it for the user's image.
