...
def make_adapter(*, host: str = "127.0.0.1", port: int = 0, with_message_handler: bool = True):
    ...
    adapter = module.KissneMobileAdapter(config, Platform(PLATFORM))
    if with_message_handler:
        async def accept_for_transport_test(_event):
            return None

        adapter.set_message_handler(accept_for_transport_test)
    return adapter
